...
Original file is located at
    https://colab.research.google.com/drive/1Tbb9F3oiglHcogEaKFpqoaezYNTEUq72
"""
...
def sanity_checks(digits):
    wandb.log({"Image Data Shape": digits.data.shape, "Label Data Shape": digits.target.shape})
...
def visualize_test(x_test, y_test, predictions):
    ...
    wandb.log({"Visualization Test Plot": plt})
...
def evaluate_model(logisticRegr, x_test, y_test, predictions):
    ...
    wandb.log({"Mean Accuracy": score, "Balanced Accuracy": balanced_accuracy})
    wandb.log({"Precision": precesion.tolist(), "Recall": recall.tolist()})
...
def main():
    wandb.init(project="task1", name="training_trials")
    ...
    wandb.log({"Best Balanced Accuracy": trial.value, "Best Hyperparameters": trial.params})
